...
test("consumeString", () => {
  ...
  expect(consumeString(`"typical string really"`, 0)).toEqual([22, "typical string really"]);
  expect(consumeString(`"allow stuff like \\"escapes\\""`, 0)).toEqual([
    29,
    'allow stuff like "escapes"',
  ]);
  expect(consumeString(`'Single quotes work too'`, 0)).toEqual([23, "Single quotes work too"]);
...
test("consumeNumeric", () => {
  ...
  expect(consumeNumeric("+", 0)).toEqual(null);
  expect(consumeNumeric(".", 0)).toEqual(null);
  expect(consumeNumeric(".5", 0)).toEqual([1, ["<number-token>", 0.5, "number"]]);
  expect(consumeNumeric("1", 0)).toEqual([0, ["<number-token>", 1, "integer"]]);
  expect(consumeNumeric("+3rem", 0)).toEqual([4, ["<dimension-token>", 3, "rem"]]);
  expect(consumeNumeric("-1ch", 0)).toEqual([3, ["<dimension-token>", -1, "ch"]]);
  expect(consumeNumeric(" -.5", 1)).toEqual([3, ["<number-token>", -0.5, "number"]]);
  expect(consumeNumeric("  1e-1wow", 2)).toEqual([8, ["<dimension-token>", 1e-1, "wow"]]);
  expect(consumeNumeric("  1e-1wow", 0)).toEqual(null);
  expect(consumeNumeric("3e+1", 0)).toEqual([3, ["<number-token>", 3e1, "number"]]);
  expect(consumeNumeric(".5e3% ", 0)).toEqual([4, ["<percentage-token>", 0.5e3]]);
  expect(consumeNumeric("2e10", 0)).toEqual([3, ["<number-token>", 2e10, "number"]]);
  expect(consumeNumeric("1/2", 0)).toEqual([0, ["<number-token>", 1, "integer"]]);
});
...
test("consumeIdentLike", () => {
  ...
  expect(consumeIdentLike("\\31 a2b3c", 0)).toEqual([8, "1a2b3c", "<ident-token>"]);
  expect(consumeIdentLike("\\#fake-id", 0)).toEqual([8, "#fake-id", "<ident-token>"]);
  expect(consumeIdentLike("-a-b-c-", 0)).toEqual([6, "-a-b-c-", "<ident-token>"]);
  ...
  expect(consumeIdentLike("currentColor", 0)).toEqual([11, "currentcolor", "<ident-token>"]);
  ...
  expect(consumeIdentLike("abc(   )", 0)).toEqual([3, "abc", "<function-token>"]);
  expect(consumeIdentLike("abc(    ", 0)).toEqual([3, "abc", "<function-token>"]);
  expect(consumeIdentLike("url(    ", 0)).toEqual([3, "url", "<function-token>"]);
});
...
test("old bugs", () => {
  ...
  expect(lexicalAnalysis(".dropdown-item:hover{color:#1e2125;background-color:#e9ecef}")).toEqual([
...
test("lexicalAnalysis", () => {
  const input = fs.readFileSync(path.join(process.cwd(), "lib/__fixtures__/bootstrap.css"), "utf8");
