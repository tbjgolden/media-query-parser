const TAB_CODEPOINT = 9;
const NEWLINE_CODEPOINT = 10;
const SPACE_CODEPOINT = 32;
const EXCLAMATION_CODEPOINT = 33;
const DOUBLE_QUOTE_CODEPOINT = 34;
const HASH_CODEPOINT = 35;
const PERCENTAGE_CODEPOINT = 37;
const SINGLE_QUOTE_CODEPOINT = 39;
const LEFT_PAREN_CODEPOINT = 40;
const RIGHT_PAREN_CODEPOINT = 41;
const ASTERISK_CODEPOINT = 42;
const PLUS_CODEPOINT = 43;
const COMMA_CODEPOINT = 44;
const HYPHEN_CODEPOINT = 45;
const DOT_CODEPOINT = 46;
const FORWARD_SLASH_CODEPOINT = 47;
const ZERO_CODEPOINT = 48;
const NINE_CODEPOINT = 57;
const COLON_CODEPOINT = 58;
const SEMICOLON_CODEPOINT = 59;
const LEFT_ANGLE_CODEPOINT = 60;
const RIGHT_ANGLE_CODEPOINT = 62;
const AT_SIGN_CODEPOINT = 64;
const UPPER_A_CODEPOINT = 65;
const UPPER_E_CODEPOINT = 69;
const UPPER_F_CODEPOINT = 70;
const UPPER_Z_CODEPOINT = 90;
const LEFT_SQUARE_CODEPOINT = 91;
const BACKSLASH_CODEPOINT = 92;
const RIGHT_SQUARE_CODEPOINT = 93;
const UNDERSCORE_CODEPOINT = 95;
const LOWER_A_CODEPOINT = 97;
const LOWER_E_CODEPOINT = 101;
const LOWER_F_CODEPOINT = 102;
const LOWER_Z_CODEPOINT = 122;
const LEFT_CURLY_CODEPOINT = 123;
const RIGHT_CURLY_CODEPOINT = 125;
const FIRST_NON_ASCII_CODEPOINT = 128;
export const codepointsToTokens = (codepoints, index = 0) => {
  const tokens = [];
  for (; index < codepoints.length; index += 1) {
    ...
    if (c === FORWARD_SLASH_CODEPOINT && codepoints.at(index + 1) === ASTERISK_CODEPOINT) {
      ...
      if ((prevToken === null || prevToken === void 0 ? void 0 : prevToken.type) === "whitespace") {
        tokens.pop();
      ...
      if (index + 1 < codepoints.length) {
        ...
        if (
          ...
          const flag = wouldStartIdentifier(codepoints, index + 1) ? "id" : "unrestricted";
      ...
      const result = consumeNumeric(codepoints, index);
...
export const consumeString = (codepoints, index) => {
  if (codepoints.length <= index + 1) return null;
  const c = codepoints.at(index);
  const stringCodepoints = [];
  for (let i = index + 1; i < codepoints.length; i += 1) {
    const codepoint = codepoints.at(i);
    if (codepoint === c) {
      // " end string
...
export const wouldStartIdentifier = (codepoints, index) => {
  const c = codepoints.at(index);
...
export const consumeEscape = (codepoints, index) => {
  if (codepoints.length <= index + 1) return null;
  ...
  if (cc === NEWLINE_CODEPOINT) {
    ...
    const hexCharCodes = [cc];
    const min = Math.min(index + 7, codepoints.length);
    let i = index + 2;
    for (; i < min; i += 1) {
      const ccc = codepoints.at(i);
      if (
...
export const consumeNumeric = (codepoints, index) => {
...
export const consumeNumber = (codepoints, index) => {
  const c = codepoints.at(index);
  ...
  if (index + 1 < codepoints.length) {
    ...
    if (cc === UPPER_E_CODEPOINT || cc === LOWER_E_CODEPOINT) {
      ...
      if (isValidExponent) {
        flag = "number";
        while (index < codepoints.length) {
          const ccccc = codepoints.at(index);
...
export const consumeIdentUnsafe = (codepoints, index) => {
  if (codepoints.length <= index) {
    return null;
  }
  const identChars = [];
  for (let c = codepoints.at(index); index < codepoints.length; c = codepoints.at(++index)) {
    if (
      c === HYPHEN_CODEPOINT ||
      c === UNDERSCORE_CODEPOINT ||
      (c >= UPPER_A_CODEPOINT && c <= UPPER_Z_CODEPOINT) ||
...
export const consumeUrl = (codepoints, index) => {
  ...
  while (index < codepoints.length) {
    if (codepoint === RIGHT_PAREN_CODEPOINT) {
      ...
      urlChars.push(codepoint);
    }
    codepoint = codepoints.at(++index);
  }
  return null;
};
export const consumeIdentLike = (codepoints, index) => {
