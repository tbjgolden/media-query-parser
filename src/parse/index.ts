export const parse = (mediaQuery: string): string | null => {
  try {
    ...
    console.log(JSON.stringify(prelude, null, 2))
...
export const expandShorthandMediaQuery = (mediaQuery: string): string => {
  ...
  for (let i = results.length - 1; i >= 0; i--) {
    ...
    if (!left && !right) {
      throw new Error(
        `Could not parse "${mediaQuery}" as a shorthand media query`
      )
    }
    ...
    if (right) {
      ...
      newSource += `${
        newSource === '' ? '' : ' and '
      }(${prefix}-${feat}: ${num}${rightUnit})`
    }

    mediaQuery = `${mediaQuery.slice(0, index)}${newSource}${mediaQuery.slice(
      index + sourceLength
    )}`
  }
...
export const parseMediaFeature = (mediaFeature: MediaFeature): Conditions => {
  ...
  if (value.type === 'Identifier') {
    ...
    if (feat === 'resolution') {
      if (unit !== 'dpi') {
        throw new Error(
          `This library is unable to convert "${stringValue}${unit}" to dpi`
        )
      }
    } else {
      if (unit !== 'px') {
        throw new Error(
          `This library is unable to convert "${stringValue}${unit}" to px`
        )
      }
    }
